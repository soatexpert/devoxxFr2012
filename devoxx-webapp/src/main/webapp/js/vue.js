function sendResponse(id) {
    ...
    $.ajax({
        type:"POST",
        url:"GiveResponse" + "?userName=" + userName,
...
$(function () {
    ...
    $("li[rel]:not(#play)").each(function (i) {
        ...
        $(this).overlay({
            ...
            onBeforeLoad:function () {
                monLi.addClass('liHighlight');
            },
            onClose:function () {
                monLi.removeClass('liHighlight');
            }
        });
    });
    ...
    $("#play").overlay({
        ...
        onBeforeLoad:function () {
            ...
            $.ajax({
                type:"GET",
                url:"AllQuestions" + "?userName=" + userName,
                success:function (data) {

                    if (data.ArrayList !== null) {
		                	   questions = data;
                        $("#ul_play").empty();
                        $(data).each(function (i, question) {
                            //alert(question.label);
                            $("#ul_play").append("<li class='li_question' id='" + question.id + "' >Question numero " + question.id + "</li>");
                        });
                    }
                    reloadLi();
                }
